...
TEST(Table_SimpleParser)
{
    ...
    person->create_object().set(col_person_name, "Frank").set(col_person_age, 25);
    person->create_object()
        .set(col_person_name, "Bill")
        .set(col_person_age, 30)
        .get_linklist(col_person_pet)
        .add(fido.get_key());
    person->create_object().set(col_person_name, "Ken").set(col_person_age, 35);
    person->create_object()
        ...
        .add(pluto.get_key());
    auto ll =
        person->create_object().set(col_person_name, "David").set(col_person_age, 40).get_linklist(col_person_pet);
    ...
    auto q1 = (person->column<Int>(col_person_age) < 30 || person->column<Int>(col_person_age) == 40) &&
              person->link(col_person_pet).column<String>(col_dog_name) == "Lady";
    auto descr1 = q1.get_description();
    auto q2 = person->query("(age < 30 || age == 40) and pet.name == \"Lady\"");
    auto descr2 = q2.get_description();
    CHECK_EQUAL(descr1, descr2);
    CHECK_EQUAL(q2.count(), 1);
}
